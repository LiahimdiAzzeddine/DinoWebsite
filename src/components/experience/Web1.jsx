...
export function Web1({ sectionID, isActive, lenis, ...props }) {
  ...
  useLayoutEffect(() => {
    ...
    ScrollTrigger.create({
      ...
      fastScrollEnd: true,
     onEnter: (self) => {
     console.log("🚀 ~ useLayoutEffect ~ onEnter: section1")
     },
      onLeave: (self) => {
 console.log("🚀 ~ useLayoutEffect ~ onLeave: section1")
      },
     onEnterBack:()=>{
      console.log("🚀 ~ useLayoutEffect ~ onEnterBack: section1")
     }
    ...
    ScrollTrigger.create({
      ...
      onLeave: (self) => {
        console.log("🚀 ~ useLayoutEffect ~ onLeave: web1");
        isEnteringBack = true;
        const velocityThreshold = 2000;
        const fastScroll = Math.abs(self.getVelocity()) > velocityThreshold;

        const targetY = sceneContainerGroup.current.position.y + 50;

        const animation = gsap.to(sceneContainerGroup.current.position, {
          y: targetY,
          duration: 0.8,
          ease: "sine.inOut",
          onComplete: () => {
      ...
      onEnter: (self) => {
        console.log("🚀 ~ useLayoutEffect ~ onEnter: web1");
        ...
        if (isEnteringBack) {
          let animation2 = gsap.to(sceneContainerGroup.current.position, {
      ...
      onEnterBack: (self) => {
        console.log("🚀 ~ useLayoutEffect ~ onEnterBack: web1");
        const velocityThreshold = 2000;
        const fastScroll = Math.abs(self.getVelocity()) > velocityThreshold;

        setCurrentModel(sectionID);
        disableOtherSections();
        let animation3 = gsap.to(sceneContainerGroup.current.position, {
          y: sceneContainerGroup.current.position.y - 50,
          duration: 0.3,
          ease: "circ.out",
          onComplete: () => {
